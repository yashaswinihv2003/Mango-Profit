...
T = {
    "English": {
        ...
        "hero_sub":"Fill in your details below and click Run Smart Analysis to discover your most profitable mango market.",
        "fill_details":"Your Farm Details",
        "step_village":"Step 1: Select Village","step_variety":"Step 2: Select Variety",
        "step_qty":"Step 3: Enter Quantity","step_run":"Step 4: Run Analysis",
    },
    "Telugu": {
        ...
        "step_village":"దశ 1: గ్రామం ఎంచుకోండి","step_variety":"దశ 2: రకం ఎంచుకోండి",
        "step_qty":"దశ 3: పరిమాణం నమోదు చేయండి","step_run":"దశ 4: విశ్లేషణ అమలు చేయండి",
    },
    "Hindi": {
        ...
        "step_village":"चरण 1: गाँव चुनें","step_variety":"चरण 2: किस्म चुनें",
        "step_qty":"चरण 3: मात्रा दर्ज करें","step_run":"चरण 4: विश्लेषण चलाएं",
    },
    "Kannada": {
        ...
        "step_village":"ಹಂತ 1: ಗ್ರಾಮ ಆಯ್ಕೆ","step_variety":"ಹಂತ 2: ತಳಿ ಆಯ್ಕೆ",
        "step_qty":"ಹಂತ 3: ಪ್ರಮಾಣ ನಮೂದಿಸಿ","step_run":"ಹಂತ 4: ವಿಶ್ಲೇಷಣೆ ಚಲಾಯಿಸಿ",
    },
}
...
.stButton>button:not([kind="secondary"]):hover{
    transform:translateY(-1px)!important;box-shadow:0 8px 24px rgba(212,175,55,0.42)!important;
}
...
.stButton>button[kind="secondary"]:hover{border-color:#d4af37!important;color:#d4af37!important;}
...
/* Style selectbox on main dashboard white background */
.stSelectbox>label{
...
.stSelectbox>div>div{
    ...
    border-radius:10px!important;color:#1a2e1f!important;font-size:14px!important;
}
.stNumberInput>label{
    font-size:11px!important;font-weight:600!important;color:#374151!important;
    text-transform:none!important;letter-spacing:0!important;
...
.stNumberInput>div>div>input{
    background:white!important;border:1.5px solid #d1d5db!important;
    border-radius:10px!important;color:#1a2e1f!important;
...
if not st.session_state.logged_in:
    ...
    /* Override selectbox style for login dark background */
    ...
    .stSelectbox>label{color:rgba(212,175,55,0.6)!important;}
    ...
    with mid:
        ...
        def lbl(t):
            st.markdown(f'<div style="font-size:10px;letter-spacing:2px;text-transform:uppercase;'
                        f'color:rgba(212,175,55,0.6);margin-bottom:6px;">{t}</div>',unsafe_allow_html=True)
        ...
        if st.session_state.forgot:
            ...
            for label,key_,ph_,is_pw in [
                (tx["name"],"r_nm","Full name",False),
                (tx["place"],"r_pl","Village, District",False),
...
<div style="background:linear-gradient(135deg,#040e07 0%,#0a1c10 60%,#040e07 100%);
    ...
    <div style="display:flex;align-items:center;gap:14px;">
        ...
        <div style="width:36px;height:36px;background:rgba(212,175,55,0.12);border:1.5px solid rgba(212,175,55,0.3);
            border-radius:50%;display:flex;align-items:center;justify-content:center;
            font-size:14px;font-weight:700;color:#d4af37;">{fname[0].upper()}</div>
    </div>
</div>""",unsafe_allow_html=True)

# ── SIDEBAR ──
with st.sidebar:
    st.markdown("""<div style="padding:18px 2px 10px;">
        <div style="font-size:9px;letter-spacing:3px;text-transform:uppercase;
            color:rgba(212,175,55,0.45);margin-bottom:14px;border-bottom:1px solid rgba(212,175,55,0.07);padding-bottom:8px;">
    ...
    st.markdown(f"""<div style="font-size:9px;letter-spacing:2px;text-transform:uppercase;
        color:rgba(212,175,55,0.45);margin-bottom:10px;">{tx['live_market']}</div>""",unsafe_allow_html=True)
    for v,(p,tr,cl) in {"Banganapalli":(28,"+2.1%","#22c55e"),"Totapuri":(18,"-0.8%","#ef4444"),
                         "Neelam":(22,"+1.4%","#22c55e"),"Rasalu":(30,"+3.2%","#22c55e")}.items():
        st.markdown(f"""<div style="display:flex;justify-content:space-between;padding:8px 10px;
            margin-bottom:5px;background:rgba(255,255,255,0.03);border-radius:7px;">
            <div><div style="font-size:12px;color:#e8e0d0;">{v}</div>
            <div style="font-size:10px;color:rgba(212,175,55,0.45);">₹{p}/kg</div></div>
...
<div style="background:
    linear-gradient(135deg,rgba(4,14,7,0.95) 0%,rgba(7,24,12,0.92) 100%),
    url('https://images.unsplash.com/photo-1592991538534-00e70e6a5b3f?w=1400&q=85&fit=crop');
    background-size:cover;background-position:center;
    border-radius:16px;padding:36px 40px 30px;margin-bottom:20px;
    ...
    <div style="font-size:9px;letter-spacing:4px;text-transform:uppercase;
        color:rgba(212,175,55,0.65);margin-bottom:8px;">Dashboard</div>
    <div style="font-family:'Playfair Display',serif;font-size:30px;
        color:#f5f0e8;font-weight:400;margin-bottom:6px;">{tx['welcome']}, {fname}</div>
    <div style="font-size:13px;color:rgba(245,240,232,0.5);max-width:560px;line-height:1.7;">
        {tx['hero_sub']}</div>
</div>""",unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════
# INPUT FORM  — always visible on dashboard
# ══════════════════════════════════════════════════════════════
vlist=village_list()
...
<div style="background:white;border-radius:16px;padding:28px 32px 24px;
    ...
    margin-bottom:22px;">
    <div style="font-size:13px;font-weight:700;color:#1a2e1f;margin-bottom:20px;
        display:flex;align-items:center;gap:8px;">
        <span style="font-size:18px;">🌾</span> {tx['fill_details']}
    </div>
""",unsafe_allow_html=True)

col1,col2,col3,col4=st.columns([2,1.6,1,1.4])
with col1:
    st.markdown(f'<p style="font-size:12px;font-weight:600;color:#374151;margin-bottom:4px;">{tx["step_village"]}</p>',unsafe_allow_html=True)
    sel_village=st.selectbox("village",vlist,key="sel_v",label_visibility="collapsed")
with col2:
    st.markdown(f'<p style="font-size:12px;font-weight:600;color:#374151;margin-bottom:4px;">{tx["step_variety"]}</p>',unsafe_allow_html=True)
    sel_variety=st.selectbox("variety",["Banganapalli","Totapuri","Neelam","Rasalu"],key="sel_var",label_visibility="collapsed")
with col3:
    st.markdown(f'<p style="font-size:12px;font-weight:600;color:#374151;margin-bottom:4px;">{tx["step_qty"]}</p>',unsafe_allow_html=True)
    sel_tonnes=st.number_input("tonnes",min_value=0.5,value=10.0,step=0.5,key="sel_t",label_visibility="collapsed")
with col4:
    st.markdown('<p style="font-size:12px;font-weight:600;color:#374151;margin-bottom:4px;">&nbsp;</p>',unsafe_allow_html=True)
    run_clicked=st.button(f"🔍  {tx['analyze_btn']}",key="run_btn",use_container_width=True)
    if run_clicked: st.session_state.run=True
...
<div style="background:white;border-radius:14px;padding:22px 26px;
    border:1px solid rgba(0,0,0,0.06);box-shadow:0 2px 14px rgba(0,0,0,0.04);margin-bottom:22px;">
...
for v,(p,tr,cl,desc) in {
    "Banganapalli":(28,"+2.1%","#22c55e","Best for export"),
    "Totapuri":(18,"-0.8%","#ef4444","Pulp & processing"),
    "Neelam":(22,"+1.4%","#22c55e","Local market"),
    "Rasalu":(30,"+3.2%","#22c55e","Premium table fruit"),
}.items():
    sel=sel_variety==v
    prices_html+=f"""
    <div style="text-align:center;padding:16px 10px;
        background:{'linear-gradient(135deg,#071209,#0f2318)' if sel else '#f8f6f0'};
        border-radius:12px;border:1px solid {'rgba(212,175,55,0.3)' if sel else 'transparent'};">
        <div style="font-size:11px;color:{'rgba(212,175,55,0.7)' if sel else '#6b7c6e'};margin-bottom:4px;">{v}</div>
        <div style="font-size:26px;font-weight:700;color:{'#f5f0e8' if sel else '#1a2e1f'};margin-bottom:3px;">₹{p}</div>
        <div style="font-size:12px;font-weight:600;color:{cl};margin-bottom:5px;">{tr}</div>
        <div style="font-size:10px;color:{'rgba(245,240,232,0.4)' if sel else '#a8b0aa'};">{desc}</div>
    </div>"""
st.markdown(prices_html+"</div></div>",unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════
# RESULTS — shown after Run Analysis clicked
# ══════════════════════════════════════════════════════════════
if st.session_state.get("run",False):

    with st.spinner("Analysing all markets within 150km..."):
        vlat,vlon=village_coords(sel_village)
        df_res=run_analysis(vlat,vlon,sel_variety,sel_tonnes)
    ...
    if df_res.empty:
        ...
        st.markdown(f"""
        <div style="background:linear-gradient(135deg,rgba(4,14,7,0.96),rgba(7,24,12,0.94));
            ...
            <div style="position:absolute;right:28px;top:50%;transform:translateY(-50%);
                font-size:80px;opacity:0.06;">🏆</div>
            <div style="font-size:9px;letter-spacing:3px;text-transform:uppercase;
                color:rgba(212,175,55,0.65);margin-bottom:12px;">{tx['profit_summary']}</div>
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:20px;">
                <div>
                    <div style="font-size:10px;color:rgba(245,240,232,0.4);margin-bottom:3px;">{tx['variety_lbl']}</div>
                    <div style="font-size:20px;font-weight:700;color:#f5f0e8;">{sel_variety}</div>
                </div>
                <div>
                    <div style="font-size:10px;color:rgba(245,240,232,0.4);margin-bottom:3px;">{tx['qty_lbl']}</div>
                    <div style="font-size:20px;font-weight:700;color:#f5f0e8;">{sel_tonnes} T</div>
                </div>
                <div>
                    <div style="font-size:10px;color:rgba(245,240,232,0.4);margin-bottom:3px;">{tx['best_market']}</div>
                    <div style="font-size:17px;font-weight:700;color:#d4af37;line-height:1.2;">{bd}</div>
                    <div style="font-size:10px;color:rgba(212,175,55,0.5);">{bc}</div>
                </div>
                <div>
                    <div style="font-size:10px;color:rgba(245,240,232,0.4);margin-bottom:3px;">{tx['est_profit']}</div>
                    <div style="font-size:28px;font-weight:800;color:#22c55e;">₹{bn:,}</div>
                </div>
            </div>
        </div>""",unsafe_allow_html=True)
        ...
        st.markdown(f"""<div style="font-size:9px;letter-spacing:3px;text-transform:uppercase;
            color:#6b7c6e;margin-bottom:12px;">{tx['top3']}</div>""",unsafe_allow_html=True)

        medals=[("#d4af37",tx["highest_profit"],True,"🥇"),
                ("#94a3b8",tx["second_best"],False,"🥈"),
                ("#a0785a",tx["third_best"],False,"🥉")]
        c3=st.columns(3)
        for i,(col,(acc,lbl_,top,med)) in enumerate(zip(c3,medals)):
            if i>=len(top3): break
            r=top3.iloc[i]
            with col:
                st.markdown(f"""
                <div style="background:{'linear-gradient(145deg,#040e07,#0a1c10)' if top else 'white'};
                    border-radius:16px;padding:22px;height:100%;
                    border:1px solid {acc if top else 'rgba(0,0,0,0.06)'};
                    box-shadow:{'0 12px 40px rgba(212,175,55,0.18)' if top else '0 2px 14px rgba(0,0,0,0.05)'};
                    position:relative;overflow:hidden;">
                    <div style="position:absolute;top:12px;right:14px;font-size:24px;opacity:0.12;">{med}</div>
                    <div style="font-size:9px;letter-spacing:2px;text-transform:uppercase;
                        color:{acc};margin-bottom:10px;font-weight:600;">{lbl_}</div>
                    <div style="font-size:16px;font-weight:700;
                        color:{'#f5f0e8' if top else '#1a2e1f'};margin-bottom:4px;line-height:1.3;">{r['Name']}</div>
                    <div style="display:inline-flex;align-items:center;gap:4px;
                        background:{'rgba(255,255,255,0.06)' if top else '#f3f4f6'};
                        border-radius:5px;padding:3px 9px;margin-bottom:14px;">
                        <span>{r['icon']}</span>
                        <span style="font-size:10px;color:{r['color']};font-weight:500;">{r['Type']}</span>
                    </div>
                    <div style="font-size:28px;font-weight:800;color:{acc};margin-bottom:2px;">
                        ₹{int(r['Net_Profit']):,}</div>
                    <div style="font-size:10px;color:{'rgba(245,240,232,0.3)' if top else '#9ca3af'};margin-bottom:14px;">
                        {tx['net_profit_lbl']} · {r['Dist_km']} {tx['away_lbl']}</div>
                    <div style="border-top:1px solid {'rgba(255,255,255,0.06)' if top else '#f3f4f6'};
                        padding-top:12px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;">
                        <div>
                            <div style="font-size:9px;color:{'rgba(245,240,232,0.35)' if top else '#9ca3af'};margin-bottom:2px;">{tx['revenue_lbl']}</div>
                            <div style="font-size:12px;font-weight:600;color:{'rgba(245,240,232,0.8)' if top else '#374151'};">₹{int(r['Revenue']):,}</div>
                        </div>
                        <div>
                            <div style="font-size:9px;color:{'rgba(245,240,232,0.35)' if top else '#9ca3af'};margin-bottom:2px;">{tx['transport_lbl']}</div>
                            <div style="font-size:12px;font-weight:600;color:#ef4444;">₹{int(r['Transport']):,}</div>
                        </div>
                        <div>
                            <div style="font-size:9px;color:{'rgba(245,240,232,0.35)' if top else '#9ca3af'};margin-bottom:2px;">{tx['risk']}</div>
                            <div style="font-size:12px;font-weight:600;color:#f59e0b;">{r['Risk_pct']}%</div>
                        </div>
                    </div>
                </div>""",unsafe_allow_html=True)
        ...
        <div style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border-radius:12px;
            padding:14px 20px;margin-bottom:20px;border:1px solid #fcd34d;
            ...
            <div>
                ...
                <div style="font-size:12.5px;color:#78350f;line-height:1.5;">{tx['tip_text']}</div>
            </div>
        </div>""",unsafe_allow_html=True)
        ...
        with bc1:
            st.markdown(f"""<div style="background:white;border-radius:14px;padding:20px;
                border:1px solid rgba(0,0,0,0.05);box-shadow:0 2px 14px rgba(0,0,0,0.04);">
                <div style="font-size:11px;font-weight:700;color:#1a2e1f;margin-bottom:14px;">{tx['bar_title']}</div>""",
                unsafe_allow_html=True)
        ...
        with bc2:
            st.markdown(f"""<div style="background:white;border-radius:14px;padding:20px;
                border:1px solid rgba(0,0,0,0.05);box-shadow:0 2px 14px rgba(0,0,0,0.04);">
                <div style="font-size:11px;font-weight:700;color:#1a2e1f;margin-bottom:14px;">{tx['pie_title']}</div>""",
                unsafe_allow_html=True)
            pdata=top10.groupby("Type")["Net_Profit"].sum().reset_index()
            cmap={"Mandi":"#3b82f6","Processing":"#8b5cf6","Pulp":"#f59e0b",
            ...
            fig2=go.Figure(go.Pie(
                ...
                marker=dict(colors=[cmap.get(c,"#888") for c in pdata["Type"]],
            ...
            fig2.update_layout(height=360,paper_bgcolor="white",margin=dict(l=8,r=8,t=4,b=4),
                ...
                annotations=[dict(text=f"<b>{sel_variety}</b>",x=0.5,y=0.5,
        ...
        # ── STACKED BAR ──
        st.markdown(f"""<div style="background:white;border-radius:14px;padding:20px;
            border:1px solid rgba(0,0,0,0.05);box-shadow:0 2px 14px rgba(0,0,0,0.04);margin-bottom:18px;">
            <div style="font-size:11px;font-weight:700;color:#1a2e1f;margin-bottom:14px;">
                {tx['revenue_lbl']} vs {tx['transport_lbl']} — Top 10</div>""",unsafe_allow_html=True)
        fig3=go.Figure()
        fig3.add_trace(go.Bar(name=tx["trans"],x=top10["Name"],y=top10["Transport"],
            marker_color="#ef4444",opacity=0.85,hovertemplate="%{x}: ₹%{y:,.0f}<extra></extra>"))
        fig3.add_trace(go.Bar(name=tx["net"],x=top10["Name"],y=top10["Net_Profit"],
            marker_color="#22c55e",opacity=0.9,hovertemplate="%{x}: ₹%{y:,.0f}<extra></extra>"))
        fig3.update_layout(barmode="stack",height=280,paper_bgcolor="white",plot_bgcolor="white",
        ...
        st.markdown(f"""<div style="background:white;border-radius:14px;padding:20px;
            border:1px solid rgba(0,0,0,0.05);box-shadow:0 2px 14px rgba(0,0,0,0.04);margin-bottom:18px;">
            <div style="font-size:11px;font-weight:700;color:#1a2e1f;margin-bottom:14px;">{tx['table_title']}</div>""",
            unsafe_allow_html=True)
        ...
        st.markdown(f"""<div style="background:white;border-radius:14px;padding:20px;
            border:1px solid rgba(0,0,0,0.05);box-shadow:0 2px 14px rgba(0,0,0,0.04);">
            <div style="font-size:11px;font-weight:700;color:#1a2e1f;margin-bottom:4px;">{tx['map_title']}</div>
        ...
        folium.Marker([vlat,vlon],
            popup=folium.Popup(f"<b>{fname}</b><br>📍 {sel_village}",max_width=200),
            tooltip=tx["your_loc"],
        ...
        folium.Marker([br["Lat"],br["Lon"]],
            popup=folium.Popup(
                f"<b>{br['Name']}</b><br>{br['Type']}<br>"
                f"<span style='color:#22c55e;font-weight:700'>₹{int(br['Net_Profit']):,}</span>",
                max_width=220),
