...
T = {
    "English":{
        ...
        "analyze_btn":"Find My Best Market",
        ...
        "tip_text":"Abroad Export gives the highest return (+7%) but needs quality grading. Local Export hubs are nearest and easiest to access.",
        ...
        "hero_sub":"Select your village, variety and quantity — our system analyses all nearby markets and finds your best profit.",
        "fill_details":"Enter Your Farm Details",
        "btn_village":"Select Your Village","btn_variety":"Select Variety","btn_qty":"Enter Quantity",
    },
    "Telugu":{
        ...
        "analyze_btn":"ఉత్తమ మార్కెట్ కనుగొనండి",
        ...
        "hero_sub":"మీ గ్రామం, రకం మరియు పరిమాణాన్ని ఎంచుకోండి — మా వ్యవస్థ మీకు ఉత్తమ లాభాన్ని కనుగొంటుంది.",
        "fill_details":"మీ పొలం వివరాలు నమోదు చేయండి",
        "btn_village":"మీ గ్రామం ఎంచుకోండి","btn_variety":"రకం ఎంచుకోండి","btn_qty":"పరిమాణం నమోదు చేయండి",
    },
    "Hindi":{
        ...
        "analyze_btn":"मेरा सर्वोत्तम बाज़ार खोजें",
        ...
        "hero_sub":"अपना गाँव, किस्म और मात्रा चुनें — हमारी प्रणाली आपके लिए सर्वोत्तम लाभ खोजेगी।",
        "fill_details":"अपने खेत की जानकारी दर्ज करें",
        "btn_village":"अपना गाँव चुनें","btn_variety":"किस्म चुनें","btn_qty":"मात्रा दर्ज करें",
    },
    "Kannada":{
        ...
        "hero_sub":"ನಿಮ್ಮ ಗ್ರಾಮ, ತಳಿ ಮತ್ತು ಪ್ರಮಾಣ ಆಯ್ಕೆ ಮಾಡಿ — ನಮ್ಮ ವ್ಯವಸ್ಥೆ ನಿಮಗೆ ಉತ್ತಮ ಲಾಭ ಕಂಡುಹಿಡಿಯುತ್ತದೆ.",
        "fill_details":"ನಿಮ್ಮ ಹೊಲದ ವಿವರಗಳು",
        "btn_village":"ನಿಮ್ಮ ಗ್ರಾಮ ಆಯ್ಕೆ ಮಾಡಿ","btn_variety":"ತಳಿ ಆಯ್ಕೆ","btn_qty":"ಪ್ರಮಾಣ ನಮೂದಿಸಿ",
    },
}
...
.stButton>button{
    ...
    border-radius:10px!important;padding:12px 20px!important;transition:all 0.2s!important;cursor:pointer!important;
}
.stButton>button:not([kind="secondary"]){
    background:linear-gradient(135deg,#16a34a,#15803d)!important;color:white!important;
    border:none!important;box-shadow:0 4px 16px rgba(22,163,74,0.45)!important;
}
.stButton>button:not([kind="secondary"]):hover{background:linear-gradient(135deg,#15803d,#166534)!important;transform:translateY(-1px)!important;}
.stButton>button[kind="secondary"]{background:rgba(255,255,255,0.09)!important;color:rgba(255,255,255,0.7)!important;border:1px solid rgba(255,255,255,0.22)!important;font-size:12px!important;padding:10px 14px!important;}
.stButton>button[kind="secondary"]:hover{color:white!important;border-color:rgba(255,255,255,0.5)!important;}
/* Dashboard selectbox — white background */
...
if not st.session_state.logged_in:
    ...
    .stApp{
        background-image:
            linear-gradient(135deg,rgba(2,30,8,0.80) 0%,rgba(3,46,12,0.75) 50%,rgba(2,24,6,0.88) 100%),
            url('https://images.unsplash.com/photo-1553279768-865429fa0078?w=1920&q=90&fit=crop&crop=center')!important;
        background-size:cover!important;background-position:center!important;
        background-repeat:no-repeat!important;min-height:100vh!important;
    }
    /* Login page inputs dark */
    .stTextInput>div>div>input{
        background:rgba(255,255,255,0.1)!important;border:1.5px solid rgba(255,255,255,0.22)!important;
        border-radius:10px!important;color:white!important;font-family:'Nunito',sans-serif!important;
    ...
    .stSelectbox>div>div{background:rgba(255,255,255,0.1)!important;border:1.5px solid rgba(255,255,255,0.22)!important;color:white!important;border-radius:10px!important;}
    ...
    # NAV
    st.markdown(f"""
    <div style="background:rgba(2,20,6,0.55);backdrop-filter:blur(14px);
        ...
        <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div>
                <div style="font-family:'Nunito',sans-serif;font-size:22px;font-weight:900;color:white;">{tx['title']}</div>
                <div style="font-size:10px;color:rgba(74,222,128,0.65);letter-spacing:2px;text-transform:uppercase;">{tx['tagline']}</div>
            </div>
        </div>
        <div style="font-size:11px;color:rgba(255,255,255,0.35);">AP · Telangana · Karnataka</div>
    ...
    hcol,fcol = st.columns([1.3,1])
    with hcol:
        st.markdown(f"""
        <div style="padding:60px 40px 40px 48px;">
            <div style="font-family:'Merriweather',serif;font-size:48px;color:white;font-weight:700;line-height:1.18;margin-bottom:16px;letter-spacing:-1px;text-shadow:0 2px 16px rgba(0,0,0,0.4);">
                {tx['hero_title'].replace(chr(10),'<br>')}
            </div>
            <p style="font-size:15px;color:rgba(255,255,255,0.65);line-height:1.8;max-width:440px;">{tx['hero_sub']}</p>
        </div>""", unsafe_allow_html=True)

    with fcol:
        st.markdown("""
        <div style="background:rgba(2,18,6,0.80);backdrop-filter:blur(28px);
            border:1px solid rgba(74,222,128,0.15);border-radius:20px;
            padding:36px 36px 28px;margin:28px 36px 20px 0;
            box-shadow:0 32px 80px rgba(0,0,0,0.55);">""", unsafe_allow_html=True)
        ...
        def lbl(t):
            st.markdown(f'<div style="font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:rgba(74,222,128,0.7);margin-bottom:6px;font-weight:700;">{t}</div>',unsafe_allow_html=True)
        ...
        st.markdown('<div style="text-align:center;margin-top:10px;font-size:10px;color:rgba(255,255,255,0.18);letter-spacing:1.5px;">SECURE · ENCRYPTED · YOUR DATA IS SAFE</div>',unsafe_allow_html=True)
    st.stop()
...
# NAV
st.markdown(f"""
<div style="background:linear-gradient(135deg,#052e16,#14532d);padding:0 28px;height:66px;
    display:flex;align-items:center;justify-content:space-between;
    box-shadow:0 4px 24px rgba(0,0,0,0.25);position:sticky;top:0;z-index:999;">
    <div style="display:flex;align-items:center;gap:12px;">
        <div style="width:42px;height:42px;background:linear-gradient(135deg,#16a34a,#4ade80);border-radius:11px;display:flex;align-items:center;justify-content:center;font-size:22px;">🥭</div>
        <div>
            <div style="font-family:'Nunito',sans-serif;font-size:22px;font-weight:900;color:white;">{tx['title']}</div>
            <div style="font-size:9px;color:rgba(74,222,128,0.6);letter-spacing:2.5px;text-transform:uppercase;">{tx['tagline']}</div>
        </div>
    </div>
    <div style="display:flex;align-items:center;gap:14px;">
        <div style="width:38px;height:38px;background:rgba(74,222,128,0.15);border:2px solid rgba(74,222,128,0.35);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:#4ade80;">{fname[0].upper()}</div>
        <div><div style="font-size:13px;color:white;font-weight:700;">{fname}</div><div style="font-size:10px;color:rgba(255,255,255,0.4);">📍 {fplace}</div></div>
    </div>
</div>""",unsafe_allow_html=True)

# SIDEBAR
with st.sidebar:
    st.markdown("""<div style="padding:16px 2px 10px;"><div style="font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#86efac;margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid rgba(134,239,172,0.1);">Settings</div></div>""",unsafe_allow_html=True)
    ...
    for v,(p,tr,cl) in {"Banganapalli":(28,"↑ +2.1%","#4ade80"),"Totapuri":(18,"↓ -0.8%","#f87171"),"Neelam":(22,"↑ +1.4%","#4ade80"),"Rasalu":(30,"↑ +3.2%","#4ade80")}.items():
        st.markdown(f'<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 10px;margin-bottom:5px;background:rgba(255,255,255,0.05);border-radius:8px;"><div><div style="font-size:12px;color:#dcfce7;font-weight:700;">{v}</div><div style="font-size:10px;color:#86efac;">₹{p}/kg</div></div><div style="font-size:12px;font-weight:800;color:{cl};">{tr}</div></div>',unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
# HERO — MANGO ORCHARD IMAGE
# ══════════════════════════════════════════════════════════════
st.markdown(f"""
<div style="
    background-image:
        linear-gradient(rgba(2,30,6,0.62),rgba(3,44,10,0.70)),
        url('https://images.unsplash.com/photo-1553279768-865429fa0078?w=1400&q=90&fit=crop&crop=center');
    background-size:cover;background-position:center;
    padding:52px 36px 48px;">
    <div style="max-width:600px;">
        <h1 style="font-family:'Merriweather',serif;font-size:42px;color:white;font-weight:700;
            line-height:1.2;margin-bottom:12px;text-shadow:0 2px 12px rgba(0,0,0,0.4);">
            {tx['hero_title']}
        </h1>
        <p style="font-size:15px;color:rgba(255,255,255,0.72);line-height:1.75;text-shadow:0 1px 8px rgba(0,0,0,0.3);">
            {tx['hero_sub']}
        </p>
    </div>
</div>""",unsafe_allow_html=True)
...
# PRICE TICKER
st.markdown(f"""
<div style="background:#052e16;padding:11px 36px;border-bottom:1px solid rgba(74,222,128,0.1);">
    <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
        <span style="font-size:10px;font-weight:800;color:#4ade80;letter-spacing:2px;text-transform:uppercase;margin-right:10px;">{tx['live_prices']}</span>
        <span style="font-size:12px;color:white;font-weight:600;">Banganapalli <b style="color:#4ade80;">₹28</b> ↑+2.1%</span>
        <span style="color:rgba(74,222,128,0.2);margin:0 12px;">|</span>
...
# ══════════════════════════════════════════════════════════════
# INPUT FORM — inside white card, button clearly visible
# ══════════════════════════════════════════════════════════════
st.markdown('<div style="padding:0 28px 36px;background:#f0f4f0;">',unsafe_allow_html=True)

st.markdown(f"""
<div style="background:white;border-radius:16px;padding:24px 28px 22px;
    box-shadow:0 8px 32px rgba(0,0,0,0.1);border:1px solid #e5e7eb;margin-bottom:22px;">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px;
        padding-bottom:14px;border-bottom:2px solid #f0f4f0;">
        <div style="width:36px;height:36px;background:#052e16;border-radius:9px;
            display:flex;align-items:center;justify-content:center;font-size:18px;">🌾</div>
        <div style="font-size:16px;font-weight:900;color:#052e16;">{tx['fill_details']}</div>
    </div>
</div>""",unsafe_allow_html=True)

# Form row — all inside white padding so button is visible
fc1,fc2,fc3 = st.columns([2.2,1.8,1])
with fc1:
    st.markdown(f'<p style="font-size:11px;font-weight:800;color:#374151;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.5px;">📍 {tx["btn_village"]}</p>',unsafe_allow_html=True)
    sel_village=st.selectbox("__v",villages_list,key="sel_v",label_visibility="collapsed")
with fc2:
    st.markdown(f'<p style="font-size:11px;font-weight:800;color:#374151;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.5px;">🥭 {tx["btn_variety"]}</p>',unsafe_allow_html=True)
    sel_variety=st.selectbox("__var",["Banganapalli","Totapuri","Neelam","Rasalu"],key="sel_var",label_visibility="collapsed")
with fc3:
    st.markdown(f'<p style="font-size:11px;font-weight:800;color:#374151;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.5px;">⚖️ {tx["btn_qty"]}</p>',unsafe_allow_html=True)
    sel_tonnes=st.number_input("__t",min_value=0.5,value=10.0,step=0.5,key="sel_t",label_visibility="collapsed")

# Full-width green button below
...
if st.session_state.get("run",False):
    ...
    if df_res.empty:
        ...
        <div style="background:linear-gradient(135deg,#052e16,#14532d);border-radius:16px;
            padding:24px 32px;margin-bottom:22px;border:1px solid rgba(74,222,128,0.15);
            position:relative;overflow:hidden;">
            <div style="position:absolute;right:28px;top:50%;transform:translateY(-50%);font-size:80px;opacity:0.06;">🏆</div>
            <div style="font-size:9px;letter-spacing:3px;text-transform:uppercase;color:#4ade80;margin-bottom:10px;font-weight:800;">{tx['profit_summary']}</div>
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:20px;">
                <div><div style="font-size:10px;color:rgba(255,255,255,0.4);margin-bottom:3px;">{tx['variety_lbl']}</div><div style="font-size:20px;font-weight:900;color:white;">{rvar}</div></div>
                <div><div style="font-size:10px;color:rgba(255,255,255,0.4);margin-bottom:3px;">{tx['qty_lbl']}</div><div style="font-size:20px;font-weight:900;color:white;">{rt} Tonnes</div></div>
                <div><div style="font-size:10px;color:rgba(255,255,255,0.4);margin-bottom:3px;">{tx['best_market']}</div><div style="font-size:16px;font-weight:800;color:#4ade80;line-height:1.25;">{bd}</div><div style="font-size:10px;color:rgba(255,255,255,0.4);">{bc}</div></div>
                <div><div style="font-size:10px;color:rgba(255,255,255,0.4);margin-bottom:3px;">{tx['est_profit']}</div><div style="font-size:32px;font-weight:900;color:#4ade80;">₹{bn:,}</div></div>
            </div>
        </div>""",unsafe_allow_html=True)
        ...
        for i,(col,(acc,lbl_,istop,med,bg)) in enumerate(zip(c3,medals)):
            ...
            with col:
                ...
                <div style="background:{bg};border-radius:16px;padding:22px;height:100%;
                    ...
                    <div style="font-size:9px;letter-spacing:2px;text-transform:uppercase;color:{acc};margin-bottom:10px;font-weight:900;">{lbl_}</div>
                    ...
                    <div style="font-size:11px;color:{'rgba(255,255,255,0.35)' if istop else '#9ca3af'};margin-bottom:16px;">{r['Dist_km']} {tx['away_lbl']}</div>
                    <div style="border-top:1px solid {'rgba(255,255,255,0.07)' if istop else '#f3f4f6'};padding-top:14px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;">
                        <div><div style="font-size:9px;color:{'rgba(255,255,255,0.38)' if istop else '#9ca3af'};margin-bottom:2px;">{tx['revenue_lbl']}</div><div style="font-size:12px;font-weight:800;color:{'rgba(255,255,255,0.85)' if istop else '#16a34a'};">₹{int(r['Revenue']):,}</div></div>
                        <div><div style="font-size:9px;color:{'rgba(255,255,255,0.38)' if istop else '#9ca3af'};margin-bottom:2px;">{tx['transport_lbl']}</div><div style="font-size:12px;font-weight:800;color:#ef4444;">₹{int(r['Transport']):,}</div></div>
                        <div><div style="font-size:9px;color:{'rgba(255,255,255,0.38)' if istop else '#9ca3af'};margin-bottom:2px;">{tx['risk']}</div><div style="font-size:12px;font-weight:800;color:#f59e0b;">{r['Risk_pct']}%</div></div>
                    </div>
                </div>""",unsafe_allow_html=True)
        ...
        # TIP
        st.markdown(f"""
        <div style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border-radius:12px;padding:14px 20px;margin-bottom:22px;border-left:4px solid #f59e0b;display:flex;align-items:flex-start;gap:12px;">
            <span style="font-size:20px;flex-shrink:0;">💡</span>
            <div><div style="font-size:11px;font-weight:900;color:#92400e;letter-spacing:1px;text-transform:uppercase;margin-bottom:3px;">{tx['farmer_tip']}</div>
            <div style="font-size:13px;color:#78350f;line-height:1.6;">{tx['tip_text']}</div></div>
        </div>""",unsafe_allow_html=True)
        ...
        with cc1:
            ...
            fig=go.Figure(go.Bar(y=top3["Name"],x=top3["Net_Profit"],orientation="h",
                ...
                textfont=dict(size=12,color="#374151",family="Nunito"),
                hovertemplate="<b>%{y}</b><br>₹%{x:,.0f}<extra></extra>"))
            fig.update_layout(height=220,paper_bgcolor="white",plot_bgcolor="white",margin=dict(l=0,r=90,t=4,b=4),font=dict(family="Nunito"),
                xaxis=dict(showgrid=True,gridcolor="#f0f4f0",zeroline=False,tickfont=dict(size=10,color="#9ca3af"),tickprefix="₹"),
                yaxis=dict(autorange="reversed",tickfont=dict(size=12,color="#052e16")),
        ...
        st.markdown('<div style="font-size:11px;color:#6b7280;margin-bottom:12px;">🏠 Your farm &nbsp;&nbsp;★ Best market &nbsp;&nbsp;Colored dots = other markets</div>',unsafe_allow_html=True)
        ...
        folium.Marker([vlat,vlon],
            ...
            icon=folium.DivIcon(html='<div style="background:#052e16;border:3px solid #4ade80;border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;font-size:18px;box-shadow:0 3px 14px rgba(0,0,0,0.5);">🏠</div>',icon_size=(40,40),icon_anchor=(20,20))
        ).add_to(m)

        rcols=["#f59e0b","#9ca3af","#b45309"]
        for i,(_,row) in enumerate(top3.iterrows()):
            coords,_ = osm_route(vlat,vlon,row["Lat"],row["Lon"])
            folium.PolyLine(coords,color=rcols[i],weight=5 if i==0 else 3,opacity=0.92 if i==0 else 0.55,dash_array=None if i==0 else "8 5").add_to(m)
            if i==0:
                folium.Marker([row["Lat"],row["Lon"]],
                    ...
                    tooltip=f"★ {row['Name']}",
                    icon=folium.DivIcon(html='<div style="background:#f59e0b;border:3px solid white;border-radius:50%;width:42px;height:42px;display:flex;align-items:center;justify-content:center;font-size:21px;font-weight:900;color:#1a1a1a;box-shadow:0 4px 16px rgba(245,158,11,0.65);">★</div>',icon_size=(42,42),icon_anchor=(21,21))
                ).add_to(m)
            else:
                folium.CircleMarker([row["Lat"],row["Lon"]],radius=11,color=rcols[i],fill=True,fill_color=rcols[i],fill_opacity=0.9,weight=3,
...
<div style="background:#052e16;padding:20px 32px;margin-top:24px;text-align:center;">
    ...
    <div style="font-size:10px;color:rgba(74,222,128,0.35);letter-spacing:1px;">© 2025 · {tx['tagline']} · AP · Telangana · Karnataka</div>
</div>""",unsafe_allow_html=True)
